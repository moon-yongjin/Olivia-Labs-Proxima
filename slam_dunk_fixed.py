...
def generate_and_animate():
    for i, prompt in enumerate(prompts):
        ...
        time.sleep(5) # Wait more for UI reset
        ...
        # [Step 2] Polling for the Animate button (Wait up to 60s)
        print(f"Waiting for image and Animate button for Scene {i+1}...")
        found = False
        for attempt in range(12): # 12 * 5s = 60s max wait
            ...
            if res and res.get('result') != 'Not found':
                ...
                found = True
                break
            print(f"Attempt {attempt+1}: Button not found yet, waiting...")
            
        if not found:
            print(f"Failed to find Animate button for Scene {i+1} after 60s")
        
        print(f"Scene {i+1} sequence finished.\n")
        time.sleep(3)
